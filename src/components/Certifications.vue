...
<template>
  <div class="bg-white dark:bg-gray-800 h-screen h-full py-6 sm:py-8 lg:py-12">
    <div class="mx-auto max-w-screen-2xl px-4 md:px-8">
      <div class="mb-4 flex items-center justify-between gap-8 sm:mb-8 md:mb-12">
        <div class="flex items-center gap-12">
          <h2 class="text-2xl font-bold text-gray-800 lg:text-3xl dark:text-white">
            Diz Andriana Certifications
          </h2>

          <p class="hidden max-w-screen-sm text-gray-500 dark:text-gray-300 md:block">
            Here you will find all the certifications I have had on the
            <a class="text-blue-600 underline" href="https://skillvalue.com/" target="_blank"
              >skillValue
            </a>
            platform
          </p>
      ...
      <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:gap-6 xl:gap-8">
        ...
        <a
          href="#"
          v-for="(img, index) in certifs"
          :key="index"
          class="group relative flex h-48 items-end overflow-hidden rounded-lg bg-gray-100 shadow-lg md:h-80"
          @click="openModal(img)"
        >
          <img
            :src="`https://github.com/AndrianD/certifications/blob/main/${img}?raw=true&auto=format&q=75&fit=crop&w=600`"
            loading="lazy"
  ...
  >
    <div class="bg-white p-6 rounded-lg shadow-xl">
      <div
        @click="closeModal"
        class="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full bg-gray-200 hover:bg-gray-300 cursor-pointer"
      >
